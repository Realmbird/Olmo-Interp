...
from data_models import UniverseContext, SynthDocument
from utils import load_txt, parse_tags, load_jsonl, load_json
...
class SyntheticDocumentGenerator:
    def __init__(
        ...
        doc_gen_global_context_path: str = "prompts/doc_gen_global_context.txt",
        batch_model: str = "gpt-4o-mini",
        oai_batch_log_dir_path: str = "oai_batch_logs",
    ):
        self.model = model
    ...
    async def brainstorm_doc_type(self, fact: str | None, num_doc_types: int = 50):
        if fact:
            prompt = load_txt(
                "prompts/brainstorm_doc_type.txt"
            )
            prompt = prompt.format(fact=fact)
        else:
            prompt = load_txt(
                "prompts/brainstorm_doc_type_from_uni_context.txt"
            )
    ...
    async def brainstorm_doc_ideas(
        ...
        if fact:
            prompt = load_txt(
                "prompts/brainstorm_doc_idea.txt"
            )
            ...
            prompt = load_txt(
                "prompts/brainstorm_doc_idea_from_uni_context.txt"
            )
            prompt = prompt.format(
                document_type=document_type,
                additional_text=additional_text if additional_text else "",
    ...
    async def generate_doc(
        ...
        prompt = load_txt("prompts/gen_doc.txt")
        prompt = prompt.format(
            fact=fact,
            document_type=document_type,
            idea=document_idea,
    ...
    async def batch_generate_documents_from_doc_specs(
        ...
        if use_facts:
            ...
            prompt = load_txt(
                "prompts/gen_doc_from_uni_context.txt"
            )
